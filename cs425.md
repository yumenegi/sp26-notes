...
- [CS 425](#cs-425)
  - [01/21/25](#012125)
    ...
    - [How Processes Communicate](#how-processes-communicate)
    - [Communication Channel properties](#communication-channel-properties)
    - [Syncronous Distributed Systems](#syncronous-distributed-systems)
    - [Asynchronous](#asynchronous)
    - [Real Life Systems](#real-life-systems)
    - [Detect a crashed process](#detect-a-crashed-process)
    - [Periodic Ping](#periodic-ping)
    - [Heartbeats](#heartbeats)
    - [Correctness of Failure Detection](#correctness-of-failure-detection)
    - [Metric for failure detection](#metric-for-failure-detection)
    - [Type of Failure](#type-of-failure)
  - [01/28/26](#012826)
    - [Omission](#omission)
    - [Two Generals Problem](#two-generals-problem)
    - [Arbitrary/Byzantine Failures](#arbitrarybyzantine-failures)
    - [Timing Failures](#timing-failures)
    - [Centralized Heartbeats](#centralized-heartbeats)
    - [Ring Heartbeating](#ring-heartbeating)
    - [All-to All Heartbeats](#all-to-all-heartbeats)
    - [Clocks](#clocks)
    - [Clock Synchronization](#clock-synchronization)
    - [Synchronization Bound](#synchronization-bound)
    - [Synchronization in Synchronous Systems](#synchronization-in-synchronous-systems)
    - [Asynchronous Time Synchronization](#asynchronous-time-synchronization)
    - [Cristian Algorithm](#cristian-algorithm)
    - [Berkely Algorithm](#berkely-algorithm)
    - [NTP](#ntp)
    - [NTP Symmetric Synchronization](#ntp-symmetric-synchronization)
  - [01/30/26](#013026)
    - [Logical Clocks and Time Stamps](#logical-clocks-and-time-stamps)
    - [Process, State, Events](#process-state-events)
    - [Event Ordering](#event-ordering)
    - [Lamport's Logical Clock](#lamports-logical-clock)
  - [02/04/26](#020426)
    - [Vector Timestamp](#vector-timestamp)
    - [Timestamps Summary](#timestamps-summary)
    - [Global State/Snapshot](#global-statesnapshot)
    - [Notations for Global Snapshot](#notations-for-global-snapshot)
    - [Consistent Cuts and Snapshots](#consistent-cuts-and-snapshots)
    - [How to capture global state?](#how-to-capture-global-state)
    - [Chandy-Lamport Algorithm](#chandy-lamport-algorithm)
  - [02/11/26](#021126)
    - [Communication Modes](#communication-modes)
    - [Reliable Multicast](#reliable-multicast)
  - [02/13/26](#021326)
    - [Multicast Ordering](#multicast-ordering)
...
- A system with $n$ processes: $[p_1,p_2...]$
- Each $p_i$ is described by its state $s_i$ that gets transformed over time
  - State includes value of all variables, affected files etc
- Three types of events that affect $s_i$
  - Local computation
  - Sending a message
  - Receiving a message
...
- Why order events using clock and time stamps
  ...
  - Rollback recovery after failures
    - Save state of system and then log events after that
    - Rollback to checkpoint and replay events in order if system crashes
- How to order with events without perfectly synchronized clocks?
  - How do we reason about events over processes?
    - A message must be sent before it gets received 
- Happened-Before (HB)
  - Denoted by $\rarr$
  - $e\rarr e'$ means $e$ happened before $e'$
  - $e\rarr _i e'$ means $e$ happened before $e'$ as observed by $p_i$
  - If $\exists p_i, e\rarr _i e'$ then $e \rarr e'$
  - For any message m, $send(m) \rarr receive(m)$
  - If $e\rarr e'$ and $e' \rarr e''$ then $e \rarr e''$
  - If no causal relationship can be asserted, two events that cannot be asserted are said to be concurrent denoted by $a || b$
- Also called causal or potentially causal ordering

### Lamport's Logical Clock
- Logial timestamp for each event that captures the happened-before relationship
- Algorithm
  - Each process $p_i$
  - Initialize the local clock $L_i = 0$
  - Increment $L_i$ before timestamping each event
  - Piggybacks $L_i$ when sending a message
  - Upon receiving a message with clock value $t$
    - sets $L_i=max(t,L_i)$
    - Increments $L_i$ before timestamping the receive event
    - ![Lamport's Logical Clock Example](image.png)
- If $e \rarr e'$ then $L(e) < L(e')$
- If $L(e) < L(e')$ then we cannot say that $e \rarr e'$ but we can say that $e' \not \rarr e$

## 02/04/26
### Vector Timestamp
- Each event associated with vector timestamp
- Each process $p_i$ maintains vector of clocks $V_i$
- The size of this vector is the same as the number of processes
  - $V_i[j]$ is the clock for process $p_j$ as maintained by $p_i$
- If all elements of the one vector is less than the other or the same, at least one element that's strictly less, than we can say one event happened before the other
- $V < V'$ iff $V \leq V'$ & $V \leq V'$ iff $V \leq V'$ and $\exists j$ such that $V[j] < V'[j]$

### Timestamps Summary
- Comparing timestamps across events is useful
- Reconciling updates made to an onject in a distributed datastore
- Rollback recovery during failures
- High overhead with timstamps
- Physical timstamp
  - Requires clock synchronization
  - Google's Spanner Distributed Database, low round trip time, designed system around it to ensure clock synchronization
- Lamport's timestamp
  - Cannot fully differentiate between causal and concurrent ordering of events
- Vector timestamps
  - Larger message sizes

### Global State/Snapshot
- State of each process and each channel in the system at a given instant of time
- Each process has a view of the global snapshot
- Useful to capture
  - Checkpointing a system state
  - Reasoning about unreferenced objects 
  - Deadlock detections
  - Distributed debugging
- Strawman
  - Each process records its state at a certain time
    - But each process has its own clock and the state for each process has a different global snapshot
- How to capture global snapshot without clock synchronization

### Notations for Global Snapshot
- ![Notations](image-1.png)
- Cut
- ![Cut](image-2.png)
  - Frontier of $C_1={a,e}$
- A run is a total ordering of events in $H$ that is consistent with each $h_i$ ordering
  - Don't care about the ordering across processes
- A linearization is a run consistent with happens-before relation in H
  - Valid across processes

### Consistent Cuts and Snapshots
- A cut is consistent if and only if for all events e, that are part of the cut, if there's an event that happened before e, it should also be included in the cut
- A global state is only consistent if the cut corresponding to it is also consistent 

### How to capture global state?
- Find a consistent global state
  - For a system with n processes, capture the state of the system after the $c_i$th event at process $p_i$
    - State corresponding to the cut defined by frontier events
- We want the state to be consistent 
  - Must be a consistent cut

### Chandy-Lamport Algorithm
- Goal
  - Record a consistent global snapshot
  - Identify a consistent cut
  - Records state locally at each process
- System model and assumptions
  - System of n processes
  - There are twon uni-directional communication channels between each ordered process
  - Communication channels are FIFO-ordered
    - If $p_i$ sends a $m$ before $m'$ to $p_j$ then $p_j$ receives $m$ before $m'$
    - Messages are received in order, no out of order
  - All messages arrive intact and are not duplicated
  - No failures
  - TCP will enforce a FIFO order
  - Snapshot should not interfere with normal applications
- Strawman approach
  - $p_i$ initiate a capture
    - Capture its own state
    - Create a special marker message and send to all other process
  - When a process receives a marker they record their own states
  - Frontiers of the cut created by strawman is not consistent
  - ![Inconsistent Capture](image-3.png)
- Improved Method
  - - $p_i$ initiate a capture
    - Capture its own state
    - Create a special marker message and send to all other process
    - Start recording messages recieved on other channels until a marker is received on a channel
  - When a process receives a marker they record their own states
    - The first marker it receives will start the capture and relaying
    - Sends markers to all other channels
      - ![FIFO](image-4.png)
    - Start recording messages recieved on other channels until a marker is received on a channel
    - Due to FIFO, this leads to a consistent cut
- Channel state recording
  - How to capture all the channel states consistently?
    - Start capturing state and send capture command
    - Start capturing own message receiving afterwards pending on channel
    - Once receiving capture command from the pending channel, stop recording receive commands
    - Receive events are not part of the state
      - Sent out but not received, channel state
      - If I received message from a server, before receiving the corresponind marker 
- ![Chandy 1](image-5.png)
- ![Chandy 2](image-6.png)
- The algorithm terminates when all process have received a marker to record their own state
- All processes have received a marker on all the $(n-1)$ incoming channels to record the state of all channels
- Don't want to interrupt the running distributed application
- Obeys causality
...
### Communication Modes
- Unicast
  - Messges are sent from one process to another
...
- Broadcast
  - Message are sent to every process
- Multicast
  - Message broadcast within a group
  - Message from one process to a group
  - Distributed Storage
    - Write to an object that are in replica servers
...
- Validity
  - If a cirrect process multiucasts, the it will evnetually deliver m to itself
  - Liveness for the sender
- Agreement
  - If a correct process delivers message, then all the other correct process in group will eventally deliver the message
    - All or nothing
- Implementation
  - Basic Multicast
    - Striaghtforward way to implement B-multicast
      - Use a reliable one-to-one send operation
        - Send $m$ to each process $p$ in group $g$
        - Receive $m$ at $p$
    - Guarantees
      - Message is eventually delivered to the group if processes are non-faulty
      - The unicast send is reliable
      - Sender does not crash
  - Reliable Multicast
    - Integrity: A correct process p delivers a message m at most once
      - Assumption: no process sends exactly the same message twice
    - Validity: If a correct process multicasts message m then it will eventually deiver m to itself
    - Agreement: If a correct process delivers message m then all the other correct processes in group of the message will eventually deliver m
...
- ![Multicast](image-9.png)
  - When message is received, union with received set
  - If never seen before, multicast it out
## 02/13/26
### Multicast Ordering
- FIFO
  - If a process issues a multicast and the another mutlicast, then every correct process that delivers the new message would have delivered the older message
- Causal
  - Follows causal order for all processes
  - ![Causality for HB Multicast](image-10.png)
